 ...
 */
export function getFilesForUpload(
  scanDirectory: string,
  shouldRecurse: boolean,
  baseContainerPath: string,
  extensionsToUpload: Array<string>
): Array<LocalFileMapping> {

  let filesToUpload: Array<string> = [];
  q.enqueue(scanDirectory);
  while (!q.isEmpty()) {
    const currentDirectoryPath = q.dequeue();
    console.log('Traversing directory: ', currentDirectoryPath);
    const currentDirectoryContents: Array<string> = fs.readdirSync(currentDirectoryPath);
    ...
    let uploadCandidates: Array<string> =
  ...
  let uploadStructure: Array<LocalFileMapping> = filesToUpload.map(t => {
